...
- [ ] **Interface Python** : Créer `class MemoryStore(ABC)` avec toutes les méthodes abstraites
...
- [ ] **Cohérence avec vectorstore.py** : Même pattern que `VectorStoreAdapter` (Story 6.2 référence)
- [ ] **Docstrings complètes** : Signature, Args, Returns, Raises pour chaque méthode abstraite

### AC2: Implémentation PostgreSQL renommée

- [ ] **Renommage** : `MemorystoreAdapter` → `PostgreSQLMemorystore(MemoryStore)`
- [ ] **Héritage** : Implémenter toutes les méthodes abstraites de `MemoryStore`
- [ ] **Zero régression** : Code existant (Stories 6.1/6.2) reste fonctionnel
- [ ] **Imports** : Tous les imports dans codebase mis à jour (`graph_populator.py`, tests, etc.)

### AC3: Factory pattern amélioré

- [ ] **Factory** : `get_memorystore_adapter() -> MemoryStore` (retourne interface, pas implémentation)
- [ ] **Config** : Variable env `MEMORYSTORE_PROVIDER` (default: `postgresql`)
- [ ] **Extensibilité** :
  ```python
  ...
  ```
- [ ] **Documentation** : Guide migration provider (`docs/memorystore-provider-migration.md`)

### AC4: Tests unitaires avec mocks (isolation PostgreSQL)

- [ ] **Tests interface** : Test que `PostgreSQLMemorystore` implémente tous les `@abstractmethod`
- [ ] **Mocks asyncpg** : 15+ tests avec mocks (pas de PostgreSQL réel)
- [ ] **Coverage** : >=90% sur factory + classe abstraite
- [ ] **Tests existants** : Refactorés pour utiliser interface (pas implémentation directe)
- [ ] **CI/CD** : Tests mocks rapides (<5s) dans CI, tests intégration PostgreSQL réels séparés

### AC5: Documentation migration pattern

- [ ] **Guide swap** : `docs/memorystore-provider-migration.md` (~300 lignes)
...
- [ ] **Architecture doc** : Mise à jour `architecture-friday-2.0.md` (section memorystore pattern)
- [ ] **Diagramme** : Mermaid class diagram (interface → implémentations)
...
- [ ] **Subtask 1.1**: Créer `agents/src/adapters/memorystore_interface.py`
...
- [ ] **Subtask 1.2**: Définir méthodes abstraites (10+ méthodes)
...
- [ ] **Subtask 1.3**: Docstrings détaillées par méthode
...
- [ ] **Subtask 1.4**: Types Enum partagés
...
- [ ] **Subtask 2.2**: Vérifier implémentation complète
...
- [ ] **Subtask 2.3**: Mettre à jour imports codebase
...
- [ ] **Subtask 2.4**: Tests zéro régression
...
- [ ] **Subtask 3.1**: Refactorer factory function
...
- [ ] **Subtask 3.2**: Ajouter stubs futurs providers
  ```python
...
- [ ] **Subtask 3.3**: Ajouter variable env `.env.example`
  ```bash
...
- [ ] **Subtask 4.1**: Tests interface abstraite
...
- [ ] **Subtask 4.2**: Tests factory avec mocks asyncpg
  - Mock `asyncpg.Pool`
...
- [ ] **Subtask 4.3**: Refactorer tests existants (isolation)
...
- [ ] **Subtask 4.4**: CI/CD smoke tests rapides
...
- [ ] **Subtask 5.1**: Créer guide migration (~300 lignes)
...
- [ ] **Subtask 5.2**: Diagramme Mermaid architecture
  ```mermaid
  classDiagram
      class MemoryStore {
          <<interface>>
...
- [ ] **Subtask 5.3**: Mettre à jour architecture-friday-2.0.md
...
- [ ] **Subtask 5.4**: Exemples code migration
...
- [ ] Interface abstraite `MemoryStore(ABC)` créée avec 10+ méthodes abstraites
- [ ] Classe renommée `PostgreSQLMemorystore(MemoryStore)` implémente tous `@abstractmethod`
- [ ] Factory `get_memorystore_adapter() -> MemoryStore` améliorée (multi-provider)
- [ ] Stubs futurs : Graphiti, Neo4j, Qdrant avec NotImplementedError + messages clairs
- [ ] 15+ tests interface + factory avec mocks asyncpg (pas PostgreSQL réel)
- [ ] Tests existants 40+ refactorés (zero régression)
- [ ] Coverage >=90% sur interface + factory
- [ ] Documentation `docs/memorystore-provider-migration.md` complète (~300 lignes)
- [ ] Diagramme Mermaid architecture (interface → implémentations)
- [ ] Architecture doc mise à jour (section memorystore pattern)
- [ ] `.env.example` : MEMORYSTORE_PROVIDER documenté
- [ ] Aucune régression tests existants (Stories 6.1 + 6.2)
