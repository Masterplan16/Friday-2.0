...
class PathConfig(BaseModel):
    ...
    extensions: List[str] = Field(
        ..., min_length=1, description="Extensions autorisees (.pdf, .csv, etc.)"
    )
    source_label: str = Field(
        ..., min_length=1, description="Label source (scanner_physique, csv_bancaire, etc.)"
    )
    ...
    def validate_source_label(cls, v: str) -> str:
        ...
        if not re.match(r"^[a-zA-Z0-9_-]+$", v):
            raise ValueError(f"source_label '{v}' must only contain [a-zA-Z0-9_-]")
        return v
...
class WatchdogConfigSchema(BaseModel):
    ...
    polling_interval_seconds: int = Field(
        default=1, ge=1, le=10, description="Intervalle polling (1-10s)"
    )
    stabilization_delay_seconds: float = Field(
        default=1.0, ge=0.0, le=10.0, description="Delai stabilisation fichier avant traitement"
    )
    error_directory: Optional[str] = Field(
        default=None, description="Dossier pour fichiers en erreur (sous-dossier date cree auto)"
    )
    paths: List[PathConfig] = Field(..., min_length=1, description="Dossiers surveilles")
...
class WatchdogConfigManager:
    ...
    def _load(self) -> None:
        ...
        if not self.config_path.exists():
            raise FileNotFoundError(f"Watchdog config not found: {self.config_path}")

        with open(self.config_path, "r", encoding="utf-8") as f:
            raw = yaml.safe_load(f)

        if not raw or "watchdog" not in raw:
            raise ValueError("Invalid watchdog config: missing 'watchdog' root key")
    ...
    def check_reload(self) -> bool:
        ...
        try:
            ...
            logger.warning("watchdog.config_stat_failed", config_path=str(self.config_path))
            return False
        ...
        try:
            ...
            logger.info(
                "watchdog.config_reloaded", old_paths=old_paths_count, new_paths=new_paths_count
            )
            ...
            for callback in self._on_reload_callbacks:
                try:
                    ...
                    logger.error("watchdog.reload_callback_failed", error=str(e))

            return True

        except Exception as e:
            logger.error("watchdog.config_reload_failed", error=str(e))
            return False
