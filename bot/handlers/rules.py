...
class RulesHandler:
    ...
    async def show_rule(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if not row:
            await update.message.reply_text(f"Règle `{rule_id}` introuvable.", parse_mode="Markdown")
            return
    ...
    async def delete_rule(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if not context.args:
            await update.message.reply_text("Usage: `/rules delete <rule_id>`", parse_mode="Markdown")
            return
        ...
        await update.message.reply_text(f"✅ Règle `{row['rule_name']}` désactivée.", parse_mode="Markdown")
...
def register_rules_handlers(application, db_pool: asyncpg.Pool):
    ...
    async def rules_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if subcmd == "list":
            ...
            await update.message.reply_text(f"Sous-commande `{subcmd}` inconnue. Utilise `/rules` pour aide.", parse_mode="Markdown")
