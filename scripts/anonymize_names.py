...
REPLACEMENTS = {
    ...
    (
        "SECURITY.md",
        r"\[mainteneur@exemple\.com\]\(mailto:mainteneur@exemple\.com\)",
    ): "[security contact](mailto:security@friday-project.example.com)",
...
def main():
    ...
    for py_file in root.rglob("*.py"):
        if (
            ".git" not in str(py_file)
            and "_bmad" not in str(py_file)
            and "fixtures" not in str(py_file)
        ):
