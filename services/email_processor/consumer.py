...
class EmailProcessorConsumer:
    ...
    async def connect(self):
        ...
        logger.info(
            "email_adapter_initialized", provider=os.getenv("EMAIL_PROVIDER", "imap_direct")
        )
        ...
        try:
            await self.redis.xgroup_create(
                STREAM_NAME, CONSUMER_GROUP, id="$", mkstream=True  # Start from new messages
            )
        ...
        while True:
            try:
                ...
                events = await self.redis.xreadgroup(
                    ...
                    streams={STREAM_NAME: ">"},  # '>' = new messages only (don't reprocess pending)
                    count=10,
    ...
    async def process_email_event(self, event_id: str, payload: Dict[str, str]):
        ...
        try:
            ...
            logger.info(
                "processing_email", event_id=event_id, account_id=account_id, message_id=message_id
            )
            ...
            if not email_full:
                ...
                if body_preview_anon and body_preview_anon != "[REDACTED]":
                    ...
                    await self.send_to_dlq(
                        event_id, payload, error="IMAP fetch failed after 6 retries"
                    )
                    await self.redis.xack(STREAM_NAME, CONSUMER_GROUP, event_id)
                    return
            ...
            if is_vip and vip_data:
                await update_vip_email_stats(vip_id=vip_data["id"], db_pool=self.db_pool)
            ...
            if category != "spam":
                try:
                    ...
                    valid_tasks = [
                        task for task in extraction_result.tasks_detected if task.confidence >= 0.7
                    ]
            ...
            should_draft = (
                not degraded_mode
                and category in ("professional", "medical", "academic")
                and not self._is_from_mainteneur(from_raw)
            )
            ...
            if should_draft:
                try:
                    ...
                    draft_result = await draft_email_reply(
                        email_id=str(email_id), email_data=email_data, db_pool=self.db_pool
                    )
            ...
            logger.error("email_processing_error", event_id=event_id, error=str(e), exc_info=True)
            # Ne pas XACK → message reste dans PEL pour retry

    async def fetch_email_with_retry(
        self, account_id: str, message_id: str, max_retries: int = 6
    ) -> Optional[Any]:
        """
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                if attempt > 0:
                    logger.info(
                        "imap_fetch_success_after_retry", message_id=message_id, attempt=attempt + 1
                    )
                return result
                ...
                logger.error(
                    "imap_request_error", message_id=message_id, error=str(e), attempt=attempt + 1
                )
        ...
        logger.error(
            "imap_fetch_max_retries_exceeded", message_id=message_id, max_retries=max_retries
        )
        return None
        ...
        try:
            ...
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code == 200:
                    ...
                    logger.error(
                        "dlq_alert_failed", status_code=response.status_code, response=response.text
                    )
    ...
    async def send_telegram_notification_attachments(
        self, message_id: str, from_anon: str, subject_anon: str, attachment_result: Any
    ):
        """
    ...
    async def create_event_entities(
        self, events: list, email_id: str, source_type: str, source_id: str
    ):
        """
        ...
        try:
            ...
            logger.error(
                "event_entity_creation_failed", email_id=email_id, error=str(e), exc_info=True
            )
            raise
