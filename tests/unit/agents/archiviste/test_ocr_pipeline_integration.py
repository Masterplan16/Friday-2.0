...
from agents.src.agents.archiviste.models import OCRResult, MetadataExtraction
...
async def test_pipeline_full_sequence(mock_renamer_cls, mock_extractor_cls, mock_ocr_cls,
                                       mock_ocr_result, mock_metadata):
    """
...
@patch("agents.src.agents.archiviste.pipeline.MetadataExtractor")
@patch("agents.src.agents.archiviste.pipeline.DocumentRenamer")
async def test_pipeline_rename_crash_fail_explicit(mock_renamer_cls, mock_extractor_cls,
                                                    mock_ocr_cls, mock_ocr_result, mock_metadata):
    """
    ...
    mock_extractor.extract_metadata = AsyncMock(return_value=ActionResult(
        ...
        reasoning="test",
        payload={"metadata": mock_metadata, "ocr_result": mock_ocr_result,
...
@patch("agents.src.agents.archiviste.pipeline.MetadataExtractor")
@patch("agents.src.agents.archiviste.pipeline.DocumentRenamer")
async def test_pipeline_timeout_raises_timeout_error(mock_renamer_cls, mock_extractor_cls,
                                                      mock_ocr_cls):
    """
...
@patch("agents.src.agents.archiviste.pipeline.MetadataExtractor")
@patch("agents.src.agents.archiviste.pipeline.DocumentRenamer")
async def test_pipeline_result_json_serializable(mock_renamer_cls, mock_extractor_cls,
                                                  mock_ocr_cls, mock_ocr_result, mock_metadata):
    """
    ...
    mock_ren.rename_document = AsyncMock(return_value=ActionResult(
        ...
        payload={
            "rename_result": RenameResult(
                ...
                metadata=mock_metadata, confidence=0.88, reasoning="test"
            ),
            "original_filename": "test.pdf",
            "new_filename": "2026-02-08_Facture_Test_145EUR.pdf"
        }
...
@patch("agents.src.agents.archiviste.pipeline.MetadataExtractor")
@patch("agents.src.agents.archiviste.pipeline.DocumentRenamer")
async def test_pipeline_publishes_dot_notation_events(mock_renamer_cls, mock_extractor_cls,
                                                       mock_ocr_cls, mock_ocr_result, mock_metadata):
    """
