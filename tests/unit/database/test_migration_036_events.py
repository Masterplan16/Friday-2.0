...
async def db_pool(event_loop):
    ...
    pool = await asyncpg.create_pool(
        dsn=os.getenv(
            "DATABASE_TEST_URL", "postgresql://friday:friday_dev@localhost:5432/friday_test"
        ),
        min_size=1,
        max_size=5,
    )
...
async def test_non_event_entity_without_event_properties_should_succeed(db_pool, clean_db):
    ...
    async with db_pool.acquire() as conn:
        ...
        """,
            "Dr Martin",
            json.dumps({"specialty": "cardiologue"}),
        )
...
async def test_query_upcoming_events_uses_index(db_pool, clean_db):
    ...
    async with db_pool.acquire() as conn:
        ...
        past_event = {"start_datetime": "2026-01-15T14:30:00", "status": "confirmed"}
        future_event1 = {"start_datetime": "2026-03-15T14:30:00", "status": "proposed"}
        future_event2 = {"start_datetime": "2026-04-20T10:00:00", "status": "confirmed"}
        ...
        """,
            json.dumps(past_event),
            json.dumps(future_event1),
            json.dumps(future_event2),
        )
...
async def test_query_events_by_casquette_uses_index(db_pool, clean_db):
    ...
    async with db_pool.acquire() as conn:
        ...
        """,
            json.dumps(medecin_event),
            json.dumps(enseignant_event),
        )
