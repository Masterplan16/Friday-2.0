...
class TestFetchEmailWithRetry:
    ...
    async def test_fetch_retry_backoff_exponential(self, consumer):
        ...
        with patch("asyncio.sleep") as mock_sleep:
            ...
            result = await consumer.fetch_email_from_emailengine(
                "account-test", "msg_123", max_retries=6
            )
    ...
    async def test_fetch_max_retries_exceeded(self, consumer):
        ...
        with patch("asyncio.sleep"):
            result = await consumer.fetch_email_from_emailengine(
                "account-test", "msg_123", max_retries=3
            )
...
class TestProcessEmailEvent:
    ...
    @pytest.mark.asyncio
    async def test_process_event_success_flow(
        self, consumer, sample_event_payload, sample_email_full
    ):
        ...
        consumer.redis.xack.assert_called_once_with(
            "emails:received", "email-processor-group", "event-123"
        )
    ...
    async def test_process_event_exception_no_xack(self, consumer, sample_event_payload):
        ...
        # Mock anonymize fail
        with patch(
            "services.email_processor.consumer.anonymize_text",
            side_effect=Exception("Anonymize fail"),
        ):
            await consumer.process_email_event("event-123", sample_event_payload)
