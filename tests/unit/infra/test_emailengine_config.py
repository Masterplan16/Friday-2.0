...
class TestEmailEngineConfiguration:
    ...
    def test_emailengine_service_exists(self, docker_compose_services):
        ...
        assert 'emailengine' in docker_compose_services['services'], \
    ...
    def test_emailengine_has_correct_image(self, docker_compose_services):
        ...
        assert 'postalsys/emailengine' in service['image'], \
    ...
    def test_emailengine_has_volume_for_data_persistence(self, docker_compose_services):
        ...
        has_data_volume = any(
            'emailengine-data' in vol and '/app/data' in vol
    ...
    def test_emailengine_has_database_url_env_var(self, docker_compose_services):
        ...
        assert 'environment' in service, \
            "EmailEngine service must have 'environment' section"
    ...
    def test_emailengine_has_redis_url_env_var(self, docker_compose_services):
        ...
        assert has_redis, \
            "EmailEngine must have REDIS configuration (EENGINE_REDIS or REDIS_URL)"
    ...
    def test_emailengine_has_secret_env_var(self, docker_compose_services):
        ...
        assert has_secret, \
            "EmailEngine must have EENGINE_SECRET environment variable"
    ...
    def test_emailengine_has_healthcheck(self, docker_compose_services):
        ...
        assert 'healthcheck' in service, \
            "EmailEngine must have healthcheck configured"

        healthcheck = service['healthcheck']

        assert 'test' in healthcheck, \
            "Healthcheck must have 'test' command"
        ...
        assert '/health' in test_cmd, \
            "Healthcheck test must check /health endpoint"
    ...
    def test_emailengine_has_restart_policy(self, docker_compose_services):
        ...
        assert 'restart' in service, \
            "EmailEngine must have restart policy"
    ...
    def test_emailengine_connected_to_friday_network(self, docker_compose_services):
        ...
        assert 'networks' in service, \
            "EmailEngine must be connected to a network"

        assert 'friday-network' in service['networks'], \
    ...
    def test_emailengine_port_not_exposed_publicly(self, docker_compose_services):
        ...
        assert 'ports' in service, \
            "EmailEngine must have port mapping"

        # Vérifier que le port est lié à 127.0.0.1
        ports = service['ports']
        has_localhost_binding = any(
            '127.0.0.1:3000' in str(port) or 'localhost:3000' in str(port)
            for port in ports
        )
